calcdifflogon <- function(df, timing = 'week', stattype = 'mean')
{
  #browser()
  cat('Timing:', timing, '\n')
  ...
  ## Exclude logonduration that is 0 and < 0. Both are invalid
  df.prevday <- subset(df.prevday, logonduration > 0)
  df.currday <- subset(df.currday, logonduration > 0)
  ...
  ##Calculate SD
	...
	## Print unique username to avoid calculating it manually
	print(paste("Unique User Count Previous Week: ",length(unique(df.prevday$UserName))))
	print(paste("Unique User Count This Week: ",length(unique(df.currday$UserName))))
  ...
  stateline.df <- data.frame(long=stateline.long, lat=stateline.lat)
	...
	g.prevmean <- ggplot() + 
	          ...
	          scale_fill_gradientn(colours=c('green4','green2', 'red1','red','red'), limits = c(45,200), breaks=c(45,70,95,120,150,200))
